...
from puzzles import iter_puzzle_examples


class _ShardWriter:
    def __init__(self, out_dir: str, split: str, shard_size: int, with_meta: bool = False):
...
def main() -> None:
    ...
    train_writer = _ShardWriter(
        args.out_dir, split="train", shard_size=args.shard_size, with_meta=False
    )
    val_writer = _ShardWriter(
        args.out_dir, split="val", shard_size=args.shard_size, with_meta=True
    )
    ...
    seen = 0
    t0 = time.time()
    for ex in iter_puzzle_examples(args.puzzles_csv, limit=(args.limit or None)):
        seen += 1
        is_val = _is_val_key(ex.fen_key, seed=args.seed, val_ratio=val_ratio)
        ...
        if is_val:
            if not _cap_reached(val_cap_samples, val_writer):
                val_writer.add(
                    ...
                    ex.legal_mask,
                    fen=ex.fen,
                    move_uci=ex.best_move_uci,
                    puzzle_id=ex.puzzle_id,
                )
        else:
            if not _cap_reached(train_cap_samples, train_writer):
                train_writer.add(ex.state, ex.target_index, ex.legal_mask)

        # Stop reading early if both configured caps are reached.
